...
// check the app key
$result = $mysqli->query("SELECT id FROM `vars` WHERE var_name='callback_app_key' AND var='".$appkey."';");
if ($result->num_rows == 0) {
  // log error and exit script
  $mysqli->query("INSERT INTO debug_log (`id`, `date`, `text`) VALUES (NULL, NOW(), 'err: app_key not valid, connect from IP: ".$stat_ip."/".$ua."');");
  exit();
}
...
// check json decoding
if ($data === null) {
  $mysqli->query("INSERT INTO debug_log (`id`, `date`, `text`) VALUES (NULL, NOW(), 'err: json_decode return null json: ".$cb_msg."');");
  exit();
} else {
  $mysqli->query("INSERT INTO debug_log (`id`, `date`, `text`) VALUES (NULL, NOW(), 'log: json_decode ok');");
}
...
// get object_id
$result = $mysqli->query("SELECT object_id FROM `objects` WHERE modem_id='".$uid."';");
if ($result->num_rows == 0) {
  // log error and exit script
  $mysqli->query("INSERT INTO debug_log (`id`, `date`, `text`) VALUES (NULL, NOW(), 'err: unknown object uid: ".$uid."');");
  exit();
}
